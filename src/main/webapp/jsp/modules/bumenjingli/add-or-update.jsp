...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑部门经理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>部门经理管理</span></li>
								<li class="breadcrumb-item active"><span>编辑部门经理</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">部门经理信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>经理工号</label>
										<input id="jingligonghao" name="jingligonghao" class="form-control" placeholder="经理工号">
									</div>
									<div class="form-group col-md-6">
										<label>密码</label>
									...
									<div class="form-group col-md-6">
										<label>经理姓名</label>
										<input id="jinglixingming" name="jinglixingming" class="form-control" placeholder="经理姓名">
									</div>
									<div class="form-group col-md-6">
										<label>性别</label>
									...
									<div class="form-group col-md-6">
										<label>部门名称</label>
										<select id="bumenmingchengSelect" name="bumenmingcheng" class="form-control">
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>职务</label>
										<input id="zhiwu" name="zhiwu" class="form-control" placeholder="职务">
									</div>
									<div class="form-group col-md-6">
										<label>邮箱</label>
	...
	<script>
		...
		var bumenmingchengOptions = [];

				function bumenmingchengSelect(){
					http("option/bumenleixing/leixing","GET",{},(res)=>{
						if(res.code == 0){
							bumenmingchengOptions = res.data;
							var nullOption = document.createElement('option');
							var bumenmingchengSelect = document.getElementById('bumenmingchengSelect');
							bumenmingchengSelect.appendChild(nullOption);
							for(var i=0;i<bumenmingchengOptions.length;i++){
								var bumenmingchengOption = document.createElement('option');
								bumenmingchengOption.setAttribute('name','bumenmingchengOption');
								bumenmingchengOption.setAttribute('value',bumenmingchengOptions[i]);
								bumenmingchengOption.innerHTML = bumenmingchengOptions[i];									
								if(ruleForm.bumenmingcheng == bumenmingchengOptions[i]){
									bumenmingchengOption.setAttribute('selected','selected');
								}
								bumenmingchengSelect.appendChild(bumenmingchengOption);
							}
						}
					});
				}	
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "bumenjingli/info/" + id,
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("bumenjingli/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("bumenjingli/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#jingligonghao").val(ruleForm.jingligonghao);	
			$("#mima").val(ruleForm.mima);	
			$("#jinglixingming").val(ruleForm.jinglixingming);	
			...
			var bumenmingchengOptions = document.getElementsByClassName("bumenmingchengOption");
			for(var bumenmingchengCount = 0; bumenmingchengCount < bumenmingchengOptions.length;bumenmingchengCount++){
				if(bumenmingchengOptions[bumenmingchengCount].getAttribute('value') == ruleForm.bumenmingcheng){
					bumenmingchengOptions[bumenmingchengCount].setAttribute('selected','selected');
				}
			}
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.jingligonghao != null && res.data.jingligonghao != ''){

					$("#jingligonghao").val(res.data.jingligonghao);
					}
					if(res.data != null && res.data != '' && res.data.mima != null && res.data.mima != ''){

					$("#mima").val(res.data.mima);
					}
					if(res.data != null && res.data != '' && res.data.jinglixingming != null && res.data.jinglixingming != ''){

					$("#jinglixingming").val(res.data.jinglixingming);
					}
					...
					var bumenmingchengOptions = document.getElementsByClassName("bumenmingchengOption");
						for(var bumenmingchengCount = 0; bumenmingchengCount < bumenmingchengOptions.length;bumenmingchengCount++){
							if(bumenmingchengOptions[bumenmingchengCount].getAttribute('value') == res.data.bumenmingcheng){
								bumenmingchengOptions[bumenmingchengCount].setAttribute('selected','selected');
							}
						}
					}
