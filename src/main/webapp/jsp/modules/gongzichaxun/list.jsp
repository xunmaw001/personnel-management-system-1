...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">工资查询管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>工资查询管理</span></li>
								<li class="breadcrumb-item active"><span>工资查询列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">工资查询列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<select name="gongziyuefen" id="gongziyuefenSelect" class="form-control form-control-sm" aria-controls="tableId">
										<option value=" "> </option>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('gongziyuefen')">工资月份<i id="gongziyuefenIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('jibengongzi')">基本工资<i id="jibengongziIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('jixiaogongzi')">绩效工资<i id="jixiaogongziIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('jiabangongzi')">加班工资<i id="jiabangongziIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('quanqin')">全勤<i id="quanqinIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('chidaozaotui')">迟到早退<i id="chidaozaotuiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('qingjia')">请假<i id="qingjiaIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('geshui')">个税<i id="geshuiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('shebaodaijiao')">社保代缴<i id="shebaodaijiaoIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			var gongziyuefenIndex = document.getElementById("gongziyuefenSelect").selectedIndex;
			var gongziyuefenOptions = document.getElementById("gongziyuefenSelect").options;
			var gongziyuefenSelectedOption = gongziyuefenOptions[gongziyuefenIndex]
			if(gongziyuefenSelectedOption.text != null && gongziyuefenSelectedOption.text != ''){
				searchForm.gongziyuefen = "%" + gongziyuefenSelectedOption.text + "%";
			}
			getDataList();
		}		
		// 获取数据列表
        function getDataList() {
			http("gongzichaxun/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				...
				gongziyuefen : searchForm.gongziyuefen,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var gongziyuefenCell  = document.createElement('td');

						  gongziyuefenCell.innerHTML = item.gongziyuefen; 

			row.appendChild(gongziyuefenCell);
			...
			var jixiaogongziCell  = document.createElement('td');

						 jixiaogongziCell.innerHTML = item.jixiaogongzi;


			row.appendChild(jixiaogongziCell);
			...
			var quanqinCell  = document.createElement('td');

						 quanqinCell.innerHTML = item.quanqin;


			row.appendChild(quanqinCell);
			var chidaozaotuiCell  = document.createElement('td');

						 chidaozaotuiCell.innerHTML = item.chidaozaotui;


			row.appendChild(chidaozaotuiCell);
			var qingjiaCell  = document.createElement('td');

						 qingjiaCell.innerHTML = item.qingjia;


			row.appendChild(qingjiaCell);
			var geshuiCell  = document.createElement('td');

						 geshuiCell.innerHTML = item.geshui;


			row.appendChild(geshuiCell);
			var shebaodaijiaoCell  = document.createElement('td');

						 shebaodaijiaoCell.innerHTML = item.shebaodaijiao;


			row.appendChild(shebaodaijiaoCell);
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("gongzichaxun/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("gongzichaxun/update","POST",data,(res)=>{
				if(res.code == 0){
					$('#addRowModal').hide();
					alert("提交成功");
	...
	// 导出
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("gongzichaxun/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"gongzichaxun");
